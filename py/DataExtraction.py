...
def scrape_comments(url, page_number, file_name):
    ...
    if page.status_code != 200:
        print(f"Error fetching the page {url}. Stopping the process.")
        return False
    ...
    if not comments:  
        print(f"No Comments Found on page {page_number}. Stopping the process.")
        return False
    ...
    with open(file_name, "a+", encoding="utf-8-sig", newline='') as file:
        ...
        for i, (comment, rating) in enumerate(zip(comments, ratings), start=1):
            ...
            seller_index = full_comment_text.find("Satıcı")
            if seller_index != -1:
                parsed_comment_text = full_comment_text[seller_index + len("Satıcı"):].strip()
            else:
...
main_url = 'https://www.hepsiburada.com/xiaomi-mi-box-s-4k-android-tv-box-2nd-gen-p-hbcv00004bahgm-yorumlari'
